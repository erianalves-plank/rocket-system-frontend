...
function Crew() {
  ...
  const handleOpenModal = (operation: string) => {
    ...
    if (operation === 'Edit' && crewSelected === '')
      return;
  ...
  const handleCardClick = (crew: CrewDTO) => {
    ...
    setFormData(crew);
    console.log(`Card with ID ${crew.id} clicked`);
  };
  ...
  const cardsCrew = GetCrews().map(item => {
    return <CardContentCrew key={item.id} id={item.id} name={item.name} crewmen={item.crewmen} onClick={() => handleCardClick(item)} />
  })
  ...
  return (
    ...
    >
      ...
      <main style={theme.containerContentPage as React.CSSProperties}>
        ...
        <ButtonsManageResource handleClick={handleOpenModal} handleClickDelete={handleDeleteCrew} />
        ...
        <Modal
          ...
          {sendDataForm ? <FormCrew crew={formData} /> : <FormCrew crewmenDB={crewmenDB} />}
        </Modal>
      </main>
